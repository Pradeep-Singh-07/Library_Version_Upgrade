...
function setField(state, objectId, key, value) {
  ...
  if (!['object', 'boolean', 'number', 'string'].includes(typeof value === 'undefined' ? 'undefined' : _typeof(value))) {
    ...
    var fieldOps = OpSet.getFieldOps(state.get('opSet'), objectId, key);
    if (fieldOps.size === 1 && fieldOps.get(0).get('action') === 'set' && fieldOps.get(0).get('value') === value) {
      return state;
    } else {
...
module.exports = {
  init: init, change: change, emptyChange: emptyChange, merge: merge, diff: diff, assign: assign, load: load, save: save, equals: equals, inspect: inspect, getHistory: getHistory,
...
function instantiateProxy(opSet, objectId) {
  var objectType = OpSet.ROOT_ID === objectId ? 'makeMap' : opSet.getIn(['byObject', objectId, '_init', 'action']);
...
function rootObjectProxy(context) {
  ...
  context._get = function (objId) {
    return instantiateProxy.call(context, context.state.get('opSet'), objId);
  };
